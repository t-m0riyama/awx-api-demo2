    ...
    def route_change(self):
        ...
        if template_route.match("/login"):
            ...
            if self.session.get("user_role") == UserRole.USER_ROLE:
                ...
                match template_route.route:
                    ...
                    case "/deadline_requests":
                        ...
                        navigation_selected_index = 2
                    case "/my_requests":
                        self.formRequests = MyRequestListForm(self.session, self.page)
                    ...
                    case "/all_activities":
                        self.formRequests = AllActivityListForm(self.session, self.page)
                        page_title_new = f"{self.app_title_base} - 操作履歴"
                        navigation_selected_index = 0
                    case _:
                        Logging.error("error undefined route")
            ...
            layout = ft.ResponsiveRow(
                controls=[
                    ft.Column(
                        col={"sm": 2, "md": 2, "lg": 2, "xl": 2, "xxl": 1},
                        controls=[Sidebar(self.session, self.page, navigation_selected_index)],
                    ),
                    ft.Column(col={"sm": 10, "md": 10, "lg": 10, "xl": 10, "xxl": 11}, controls=[self.formRequests]),
                ],
                spacing=30,
