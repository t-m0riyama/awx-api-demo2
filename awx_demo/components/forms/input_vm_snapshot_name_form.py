...
class InputVmSnapshotNameForm(BaseWizardCard):
    ...
    def __init__(
        ...
        sub_title="取得するスナップショット名の入力",
        description="取得するスナップショット名を入力して下さい。",
        ...
        # controls
        formTitle = FormTitle("スナップショット名の入力", f"{sub_title}")
        formDescription = FormDescription(f"{description}＊は入力/選択が必須の項目です。")
        ...
        if self.tfVmSnapshotName.value == "":
    ...
    def on_click_next(self, e):
        # スナップショット名が指定されていない場合は、処理せず終了する
        if self.tfVmSnapshotName.value == "":
            return

        self._lock_form_controls()
        self.session.get("job_options")["target_vm_snapshot_name"] = self.tfVmSnapshotName.value
        self.session.get("job_options")["target_vm_snapshot_description"] = self.tfVmSnapshotDescription.value
