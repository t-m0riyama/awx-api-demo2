...
class CreateRequestForm(BaseWizardCard):
    ...
    def __init__(
        ...
        self.lblRequestDeadline = ft.Text(
            value=(
                (f"リリース希望日(＊): {self.session.get('request_deadline').strftime('%Y/%m/%d')}")
                if self.session.contains_key("request_deadline")
                else f"リリース希望日(＊): {start_date.strftime('%Y/%m/%d')}"
            ),
    ...
    def on_change_request_deadline(self, e):
        self.lblRequestDeadline.value = "リリース希望日: " + self.dpRequestDeadline.value.strftime("%Y/%m/%d")
