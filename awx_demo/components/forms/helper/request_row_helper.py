...
class RequestRowData:
    ...
    def __init__(
        self, request_status, request_deadline, updated, request_user, iaas_user, request_operation, request_text
    ):
...
class RequestRowHelper:
    ...
    def generate_request_row(request_list_form, row_id, request_data: RequestRowData, shortcut_index: int):
        ...
        return ft.DataRow(
            cells=[
                ...
                ft.DataCell(RequestRowHelper._request_status_to_icon(request_data.request_status)),
    ...
    def generate_request_table(
        ...
        dtRequests = ft.DataTable(
            columns=[
                ...
                ft.DataColumn(ft.Text("")),
                ft.DataColumn(ft.Text("依頼ID"), on_sort=on_sort_func),
                ft.DataColumn(ft.Text("リリース希望日"), on_sort=on_sort_func),
                ft.DataColumn(ft.Text("最終更新日"), on_sort=on_sort_func),
                ft.DataColumn(ft.Text("申請者"), on_sort=on_sort_func),
                ft.DataColumn(ft.Text("作業担当者"), on_sort=on_sort_func),
                ft.DataColumn(ft.Text("申請項目"), on_sort=on_sort_func),
                ft.DataColumn(ft.Text("依頼内容"), on_sort=on_sort_func),
            ],
            rows=[],
    ...
    def _request_deadline_to_icon(request_deadline, request_status):
        ...
        if now_date >= deadline_date:
            return ft.Icon(
                name=ft.Icons.PRIORITY_HIGH_OUTLINED, color=ft.Colors.ERROR, size=18, tooltip="リリース希望日超過"
            )
        else:
            return None
    ...
    def _request_status_to_icon(request_status):
        match request_status:
            case RequestStatus.START:
                return ft.Icon(
                    name=ft.Icons.FIBER_NEW_OUTLINED, color=ft.Colors.PRIMARY, tooltip=RequestStatus.START_FRIENDLY
                )
            ...
            case RequestStatus.APPLYING_FAILED:
                return ft.Icon(
                    name=ft.Icons.RUNNING_WITH_ERRORS_OUTLINED,
                    color=ft.Colors.ERROR,
                    tooltip=RequestStatus.APPLYING_FAILED_FRIENDLY,
                )
            case RequestStatus.COMPLETED:
                return ft.Icon(
                    name=ft.Icons.CHECK_CIRCLE, color=ft.Colors.PRIMARY, tooltip=RequestStatus.COMPLETED_FRIENDLY
                )
    ...
    def refresh_page_indicator(request_list_form):
        range_min, range_max, request_data_count = RequestRowHelper.get_page_range(request_list_form)
        request_list_form.textRequestsRange.value = "{}-{} / {}".format(range_min, range_max, request_data_count)
    ...
    def get_page_range(request_list_form):
        ...
        range_min = 0 if record_count == 0 else (request_list_form.data_row_offset + 1)
    ...
    def query_request_all(request_list_form):
        ...
        match request_list_form.session.get("sort_target_column"):
            case "依頼ID":
                order_spec = IaasRequestHelper.get_order_spec_request_id(request_list_form.dtRequests.sort_ascending)
            case "リリース希望日":
                order_spec = IaasRequestHelper.get_order_spec_request_deadline(
                    request_list_form.dtRequests.sort_ascending
                )
            case "最終更新日":
                order_spec = IaasRequestHelper.get_order_spec_updated(request_list_form.dtRequests.sort_ascending)
            case "申請者":
                order_spec = IaasRequestHelper.get_order_spec_request_user(request_list_form.dtRequests.sort_ascending)
            case "作業担当者":
                order_spec = IaasRequestHelper.get_order_spec_iaas_user(request_list_form.dtRequests.sort_ascending)
            case "申請項目":
                order_spec = IaasRequestHelper.get_order_spec_request_operation(
                    request_list_form.dtRequests.sort_ascending
                )
            case "依頼内容":
                order_spec = IaasRequestHelper.get_order_spec_request_text(request_list_form.dtRequests.sort_ascending)
