...
class TeamsMessageCardNotificator:
    ...
    def notify(cls, notification_spec: NotificationSpec):
        ...
        if notification_spec.user:
            section_attributes.addFact("ユーザ名:", notification_spec.user)
        if notification_spec.event_type:
            section_attributes.addFact("イベント種別:", EventType.to_friendly(notification_spec.event_type))
        # if notification_spec.event_type:
        #   section_attributes.addFact("依頼区分:", EventType.to_friendly(notification_spec.event_type))
        # if notification_spec.event_type:
        #   section_attributes.addFact("申請項目:", EventType.to_friendly(notification_spec.event_type))
        if notification_spec.status:
            section_attributes.addFact("ステータス:", EventStatus.to_friendly(notification_spec.status))
        if notification_spec.request_id:
            section_attributes.addFact("依頼ID:", notification_spec.request_id)
        if notification_spec.request_text:
            section_attributes.addFact("依頼内容:", notification_spec.request_text)
        if notification_spec.request_deadline:
            section_attributes.addFact("リリース希望日:", notification_spec.request_deadline.strftime("%Y/%m/%d"))
        if notification_spec.summary:
            section_attributes.addFact("概要:", notification_spec.summary)
