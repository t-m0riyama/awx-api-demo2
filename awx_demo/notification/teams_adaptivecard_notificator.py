...
class TeamsAdaptiveCardNotificator:
        ...
        try:
            if proxy_enabled:
                response = requests.post(
                    request_url, headers=headers, verify=verify, data=data, proxies=cls.AWX_PROXIES
                )
            else:
            ...
            if response.status_code == 200:
                Logging.info(
                    f"TEAMS_MESSAGE_SEND_SUCCESS: Teamsメッセージの通知に成功しました。 {notification_spec.title}"
                )
            else:
                Logging.error(
                    f"TEAMS_MESSAGE_SEND_FAILED: Teamsメッセージの通知に失敗しました。 {notification_spec.title}"
                )
    ...
    def notify(cls, notification_spec: NotificationSpec):
        ...
        title = TextBlock(
            text=notification_spec.title, size=types.FontSize.MEDIUM, weight=types.FontWeight.BOLDER, wrap=True
        )
        sub_title = TextBlock(
            text=f"{notification_spec.sub_title}", size=types.FontSize.SMALL, color=types.Colors.ACCENT, wrap=True
        )
        icon = Image(url="data:image/png;base64,{}".format(icon_base64), size="small")
        ...
        if notification_spec.user:
            facts.append(Fact(title="ユーザ名:", value=notification_spec.user))
        if notification_spec.event_type:
            facts.append(Fact(title="イベント種別:", value=EventType.to_friendly(notification_spec.event_type)))
        # if notification_spec.status:
        #   facts.append(Fact(title="依頼区分:", value=EventType.to_friendly(notification_spec.event_type)))
        # if notification_spec.status:
        #   facts.append(Fact(title="申請項目:", value=EventType.to_friendly(notification_spec.event_type)))
        if notification_spec.status:
            facts.append(Fact(title="ステータス:", value=EventStatus.to_friendly(notification_spec.status)))
        if notification_spec.request_id:
            facts.append(Fact(title="依頼ID:", value=notification_spec.request_id))
        if notification_spec.request_text:
            facts.append(Fact(title="依頼内容:", value=notification_spec.request_text))
        if notification_spec.request_deadline:
            facts.append(Fact(title="リリース希望日:", value=notification_spec.request_deadline.strftime("%Y/%m/%d")))
        if notification_spec.summary:
            facts.append(Fact(title="概要:", value=notification_spec.summary))
        ...
        card = AdaptiveCard.new().version(cls.ADAPTIVECARD_VERSION).add_items(body).create()
