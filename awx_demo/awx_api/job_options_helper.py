...
class JobOptionsHelper:
    ...
    @staticmethod
    @Logging.func_logger
    def generate_job_options(session, request_operation, for_execute_job=True):
        target_options = []
        ...
        for key in target_options:
            if key in session.get("job_options"):
                if key == "vsphere_vcenter" and for_execute_job:
                    job_options[key] = str(session.get("job_options")[key]).split("|")[1].strip()
                else:
    ...
    def get_job_template_name(request_operation):
        match request_operation:
            case RequestOperation.VM_CPU_MEMORY_CAHNGE:
                return os.getenv(
                    "JOB_TEMPLATE_SET_VM_CPU_MEMORY", JobOptionsHelper.JOB_TEMPLATE_SET_VM_CPU_MEMORY_DEFAULT
                )
            case RequestOperation.VM_START_OR_STOP:
