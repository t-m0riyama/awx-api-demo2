...
class AWXApiHelper:
    ...
    def _request_get(cls, request_url, headers, loginid, password, verify, proxy_enabled):
        if proxy_enabled:
            response = requests.get(
                ...
                verify=verify,
                proxies=cls.AWX_PROXIES,
            )
        else:
            response = requests.get(request_url, headers=headers, auth=HTTPBasicAuth(loginid, password), verify=verify)
        return response
    ...
    def _request_post(cls, request_url, headers, loginid, password, verify, data, proxy_enabled):
        if proxy_enabled:
            ...
            response = requests.post(
                request_url, headers=headers, auth=HTTPBasicAuth(loginid, password), verify=verify, data=data
            )
        return response
    ...
    def login(cls, uri_base, loginid, password):
        ...
        try:
            response = cls._request_get(
                ...
                password=password,
                verify=False,
                proxy_enabled=cls.AWX_PROXY_ENABLED,
            )
    ...
    def get_teams_user_belong(cls, uri_base, loginid, password):
        ...
        try:
            ...
            teams_url = uri_base + jmespath.search("results[0].related.teams", response_me.json())
            Logging.info(f"AWX_TEAMS_URL: {teams_url}")
            response_teams = cls._request_get(
    ...
    def start_job(cls, uri_base, loginid, password, job_template_name, request, job_options, session):
        ...
        try:
            job_template_id = cls.get_job_template_id(uri_base, loginid, password, job_template_name)
            launch_url = uri_base + "/api/v2/job_templates/{}/launch/".format(job_template_id)
    ...
    def _emit_event_on_start_job(
        ...
        title, status, summary = cls.generate_common_fields(
            request_id, EventType.REQUEST_EXECUTE_STARTED_FRIENDLY, is_succeeded, request_text, request_deadline
        )
    ...
    @classmethod
    def _emit_event_on_job_completed(
        cls,
        db_session,
        user,
        request_id,
        ...
        title, status, summary = cls.generate_common_fields(
            request_id, EventType.REQUEST_EXECUTE_COMPLETED_FRIENDLY, is_succeeded, request_text, request_deadline
        )
