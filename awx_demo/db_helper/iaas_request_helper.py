...
class IaasRequestHelper:
    ...
    def duplicate_request(cls, db_session, session, request_id, new_request_id):
        request = db_session.query(base.IaasRequest).filter(base.IaasRequest.request_id == request_id).first()
    ...
    @Logging.func_logger
    def update_request(
        cls, db_session, session, request_id, request_deadline, request_text, job_options, request_status, iaas_user
    ):
        request = db_session.query(base.IaasRequest).filter(base.IaasRequest.request_id == request_id).first()
        ...
        detail = ""
        diff_changed = IaasRequestReportHelper.generate_diff_request(
            request, request_deadline_old, request_text_old, job_options_old, request_status_old, iaas_user_old
        )
        if diff_changed:
            diff_changed_friendly = "\n== 変更内容の詳細 =============\n" + IaasRequestReportHelper.to_friendly_request(
                diff_changed
            )
        ...
        if diff_changed:
            ...
            diff_changed_except_request_status_and_iaas_user = (
                IaasRequestReportHelper.except_request_status_and_iaas_user(diff_changed)
            )
            ...
            if request_status != request_status_old:
                ...
                notification_method = (
                    NotificationMethod.NOTIFY_TEAMS_AND_MAIL
                    if request_report.request_status == RequestStatus.COMPLETED
                    else NotificationMethod.NOTIFY_TEAMS_ONLY
                )
                cls._emit_event_on_update_request_status(
                    ...
                    additional_info="({} -> {})".format(
                        RequestStatus.to_friendly(request_status_old), RequestStatus.to_friendly(request_status)
                    ),
                    detail=detail,
                    is_succeeded=True,
    ...
    def update_request_status(cls, session, db_session, request_id, request_status):
        ...
        if request_status_current != request_status:
            ...
            cls._emit_event_on_update_request_status(
                ...
                additional_info="({} -> {})".format(
                    RequestStatus.to_friendly(request_status_current), RequestStatus.to_friendly(request_status)
                ),
                detail=detail,
                is_succeeded=True,
                notification_method=notification_method,
    ...
    def get_requests(cls, db_session, filters, order_spec, offset_row, limit_rows):
        ...
        filters = list(filter(lambda x: x is not None, filters))
        requests_data = (
            db_session.query(base.IaasRequest)
            .filter(*filters)
            .order_by(order_spec)
            .offset(offset_row)
            .limit(limit_rows)
    ...
    def get_request(cls, db_session, request_id):
        request_data = db_session.query(base.IaasRequest).filter(base.IaasRequest.request_id == request_id).first()
        return request_data
    ...
    def get_filter_iaas_user_is_null(cls):
        return and_(base.IaasRequest.iaas_user is None)
    ...
    def get_filter_request_user_is_null(cls):
        return and_(base.IaasRequest.request_user is None)
    ...
    def get_filter_request_text(cls, request_text_contains):
        if request_text_contains:
            return or_(
                and_(base.IaasRequest.request_text.contains(request_text_contains)),
    ...
    def _emit_event(
        ...
        if (
            ...
            or notification_method == NotificationMethod.NOTIFY_TEAMS_AND_MAIL
        ):
        ...
        # メール通知を指定された場合
        if (
            notification_method == NotificationMethod.NOTIFY_MAIL_ONLY
            or notification_method == NotificationMethod.NOTIFY_TEAMS_AND_MAIL
        ):
            # AWX/AAPのユーザ情報を取得
    ...
    def _emit_event_on_insert(
        ...
        title, status, summary = IaasRequestReportHelper.generate_common_fields(
            request_id, EventType.REQUEST_SENT_FRIENDLY, is_succeeded, request_text, request_deadline
        )
        sub_title = "新しい申請({} / {})が作成されました。".format(request_category, request_operation)
        sub_title2 = "申請 {} の内容を確認し、必要であれば変更を行なった上で承認し、作業を実施してください。".format(
            request_id
        )
        cls._emit_event(
            db_session=db_session,
    ...
    def _emit_event_on_duplicate(
        ...
        title, status, summary = IaasRequestReportHelper.generate_common_fields(
            request_id, EventType.REQUEST_DUPLICATE_FRIENDLY, is_succeeded, request_text, request_deadline
        )
        sub_title = "申請({} / {})が複製されました。({} -> {})".format(
            request_category, request_operation, request_id, request_id_new
        )
        sub_title2 = (
            "複製した申請 {} の内容を確認し、必要であれば変更を行なった上で承認し、作業を実施してください。".format(
                request_id_new
            )
        )
        cls._emit_event(
    ...
    def _emit_event_on_update(
        ...
        title, status, summary = IaasRequestReportHelper.generate_common_fields(
            request_id, EventType.REQUEST_CHANGED_FRIENDLY, is_succeeded, request_text, request_deadline
        )
    ...
    @classmethod
    def _emit_event_on_update_request_status(
        cls,
        db_session,
        session,
        user,
        ...
        title, status, summary = IaasRequestReportHelper.generate_common_fields(
            request_id,
            EventType.REQUEST_STATUS_CHANGED_FRIENDLY,
            is_succeeded,
            request_text,
            request_deadline,
            additional_info,
    ...
    @classmethod
    def _emit_event_on_update_iaas_user(
        cls,
        db_session,
        session,
        user,
        ...
        title, status, summary = IaasRequestReportHelper.generate_common_fields(
            request_id,
            EventType.REQUEST_IAAS_USER_ASSIGNED_FRIENDLY,
            is_succeeded,
            request_text,
            request_deadline,
            additional_info,
    ...
    def _emit_event_on_delete(
        ...
        title, status, summary = IaasRequestReportHelper.generate_common_fields(
            request_id, EventType.REQUEST_DELETED_FRIENDLY, is_succeeded, request_text, request_deadline
        )
