...
class IaasRequestReportHelper:
    ...
    @staticmethod
    def diff_request_change(request, request_deadline_old, request_text_old, job_options_old, request_status_old, iaas_user_old):
        diff_request = {}
    ...
    @staticmethod
    def diff_job_options(job_options, job_options_old):
        diff_job_options = {}
    ...
    @staticmethod
    def generate_request_detail(request):
        request_report = copy.deepcopy(request)
        request_report.job_options = json.loads(request_report.job_options)
        return "\n== 申請内容の詳細一覧 =============\n" + IaasRequestReportHelper.to_friendly_request(vars(request_report))

    @staticmethod
    def generate_diff_request(request, request_deadline_old, request_text_old, job_options_old, request_status_old, iaas_user_old):
        diff_request = IaasRequestReportHelper.diff_request_change(
            request=request,
    ...
    @staticmethod
    def to_friendly_request(request, default_value=None):
        request_friendry = {}
        for i, request_key in enumerate(IaasRequestReportHelper.REQUEST_KEYS):
    ...
    @staticmethod
    def to_friendly_job_options(job_options_str, convert_to_yaml=True, default_value=None):
        job_options_dict = json.loads(job_options_str)
        for i, job_options_key in enumerate(IaasRequestReportHelper.JOB_OPTIONS_KEYS):
            if job_options_dict.get(job_options_key):
                job_options_dict[IaasRequestReportHelper.JOB_OPTIONS_FRIENDLY_KEYS[i]] = job_options_dict.pop(job_options_key, default_value)
